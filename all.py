...
# control 1 控制alpha，beta，节点数目和匹配方式
nodenumber = 50
...
# 统计结果行数
count = -1
for alpha in np.arange(alpha_min, alpha_max, alpha_gap):
    for beta in np.arange(beta_mix, beta_max, beta_gap):
        count = count + 1

# 创建存储列表
data = np.mat(np.ones((count + 1, 5)))
count = -1
# 开始运算
for alpha in np.arange(alpha_min, alpha_max, alpha_gap):
    for beta in np.arange(beta_mix, beta_max, beta_gap):
                    ...
                    if graph1.has_edge(i, j):
                        ...
                        for k in range(0, nodenumber):
                            if graph1.has_edge(i, k):
                                degree_improvement_1[i] = degree_improvement_1[i] + math.pow(
                                    (graph1.degree(k) / graph1.degree(j)), alpha)
            for i in range(0, nodenumber):
                # print("win1")
                for j in range(0, nodenumber):
                    # print("win2")
                    if graph2.has_edge(i, j):
                        ...
                        for k in range(0, nodenumber):
                            if graph2.has_edge(i, k):
                                degree_improvement_2[i] = degree_improvement_2[i] + math.pow(
                                    (graph2.degree(k) / graph2.degree(j)), alpha)
        ...
        # nx.draw_networkx_edges(G, psG, edgelist=esmall, width=1, alpha=0.21, edge_color='b',
        #                        style='dashed')  # alpha可以调整虚实
        ...
        for i in range(0, 50):
            for j in range(0, 50):
                if G.has_edge(tempGone.format(ione=i), tempGtwo.format(itwo=j)):
                    G[tempGone.format(ione=i)][tempGtwo.format(itwo=j)]['weight'] = math.pow(
                        graph1.degree(i) * graph2.degree(j), alpha)
        ...
        data[count, 0] = round(alpha, 3)
        data[count, 1] = round(beta, 3)
        data[count, 2] = round(F, 3)
        data[count, 3] = round(S, 3)
        data[count, 4] = round(P, 3)

# np.savetxt('new.csv', data, delimiter = ',')

# 选取要绘图的数据
# control 2 控制要绘图的alpha
target_alpha = 0.4
temp = np.mat(np.ones((1, 5)))

for i in range(0, count):
    if data[i, 0] == target_alpha:
        temp = np.row_stack((data[i], temp))

temp = temp[:-1]  # 删除最后一行初始值

# 绘图
# 选择变量
fsp = 3;
##control 3 控制要绘图的目标变量，fsp值为2绘制F，为3绘制S，为4绘制P
if fsp == 2:
    ylabel = 'F'
else:
    if fsp == 3:
...
plt.plot(temp[:, 1], temp[:, fsp])

plt.title('α=' + str(target_alpha))
plt.xlabel('β')
plt.ylabel(ylabel)
